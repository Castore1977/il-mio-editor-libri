...
// --- MODIFICA: Aggiunte icone per lo stato ---
import { Plus, Trash2, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, Monitor, Smartphone, FileDown, X, GripVertical, BookOpen, Users, MapPin, BarChart2, Upload, Download, Calendar, Home, Book, AlertTriangle, Loader2, LogOut, Maximize, Circle, CircleDot, CircleCheck } from 'lucide-react';
...
// --- MODIFICA: Definizione degli stati di avanzamento ---
const STATUSES = {
...
    data: {
        // --- MODIFICA: Aggiunti 'status' e 'notes' al paragrafo di default ---
...
// --- MODIFICA: Funzioni per calcolare lo stato di capitoli e libri ---
...
                        Object.values(books).sort((a, b) => b.lastModified - a.lastModified).map(book => {
                        // --- MODIFICA: Calcolo e visualizzazione dello stato del libro ---
...
            {projectData.chapters.map((chapter, cIndex) => {
                // --- MODIFICA: Calcolo e visualizzazione stato del capitolo ---
...
                        {chapter.paragraphs.map((p, pIndex) => {
                                // --- MODIFICA: Visualizzazione stato del paragrafo ---
...
                        </div>
                    </div>
                        {/* --- MODIFICA: Aggiunti campi per stato e note --- */}
                        <div className="space-y-4 my-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full p-2 border rounded-md dark:bg-gray-800 dark:border-gray-600 focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    className="w-full p-2 border rounded-md dark:bg-gray-800 dark:border-gray-600 h-24 text-sm"
                                />
                            </div>
                        </div>
                </div>
...
        if (!chapter) return data;
        // --- MODIFICA: Aggiunto 'status' e 'notes' al nuovo paragrafo ---
...
    const currentParagraphStyle = (selectedItem?.type === 'paragraph' && editorItem?.data) || { font: 'Arial', align: 'left' };
    ...
    // --- MODIFICA: Calcolo dello stato del libro attivo ---
...
                    </button>
                    <div className="text-center font-bold truncate px-4 flex items-center gap-2">
                        <Book size={16} className="inline-block mr-2 flex-shrink-0" />
                        <span className="truncate">{books[activeBookId].title}</span>
                        {/* --- MODIFICA: Visualizzazione stato del libro attivo --- */}
